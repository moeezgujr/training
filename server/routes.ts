...
import { db, schema } from './db';        // adjust path to your db.ts file
...
import { CourseDto, moduleSchema, courseSchema, enrollmentSchema, moduleContentSchema, quizSchema, certificateSchema } from "@shared/schema";
...
app.post('/api/auth/forgot-password', async (req, res) => {
  ...
  try {
    ...
    console.log('[FORGOT-PASSWORD] User exists?', !!user);
    ...
    if (user) {
      // Generate secure token (better than Math.random in production)
      const resetToken = crypto.randomBytes(32).toString('hex');

      // Use environment variable (set in .env or hosting dashboard)
      ...
      console.log('[FORGOT-PASSWORD] Email send result:', success ? 'SUCCESS' : 'FAILED');
    } else {
      console.log('[FORGOT-PASSWORD] No user found → no email sent (silent)');
    }
  } catch (err) {
    console.error('[FORGOT-PASSWORD] Error:', err);
    // Never leak error details to user
    res.status(200).json({
      message: 'If an account exists, you will receive a reset link shortly.'
    });
  }
...
app.post('/api/auth/reset-password', async (req, res) => {
  ...
  try {
    ...
    // Basic token check (expand later: store token in DB + expiry)
    // For now we just check existence - add real validation in production
    if (!token) {
    ...
    console.log(
      '[RESET-PASSWORD] New hash generated (preview):',
      hashedPassword.substring(0, 30) + '...'
    );

    // Update password using the real column name from your schema
    const updated = await db
      .update(schema.users)
      .set({ passwordHash: hashedPassword })  // ← your real column name
      .where(eq(schema.users.id, String(user.id))) // safe string conversion
      .returning();
    ...
    console.log('[RESET-PASSWORD] Password updated successfully for user ID:', user.id);
  ...
  // === QUIZ ROUTES ===
 // Submit quiz attempt – fully implemented
app.post('/api/quizzes/:quizId/submit', isAuthenticated, async (req: any, res) => {
  try {
    ...
    if (!answers || typeof answers !== 'object') {
      return res.status(400).json({ message: "Invalid or missing answers format" });
    }

    // 2. Load full quiz with questions
    ...
    // 3. Score each question with normalized comparison
    for (const question of quiz.questions) {
      const userAnswer = answers[question.id];

      // Extract correct answer - handle both string and JSON formats
      let correctAnswerValue: string | null = null;
      try {
        const raw = question.correctAnswer as any;
        if (typeof raw === 'string') {
          // Try to parse as JSON first (for legacy data)
          try {
            const parsed = JSON.parse(raw);
            correctAnswerValue = Array.isArray(parsed) ? parsed[0] : parsed;
          } catch {
        ...
        console.warn(`Invalid correctAnswer format for question ${question.id}:`, e);
      }

      let isCorrect = false;

      if (correctAnswerValue !== null && userAnswer) {
        // Normalize both answers for comparison
        const normalizedUserAnswer = userAnswer.toString().trim().toLowerCase();
        const normalizedCorrectAnswer = correctAnswerValue.toString().trim().toLowerCase();
        ...
        if (question.questionType === 'multiple_choice') {
          ...
          // For fill in the blank, exact match after normalization
          isCorrect = normalizedUserAnswer === normalizedCorrectAnswer;
        }
      }
  ...
  try {
    ...
    // Load course with full modules + quizzes + questions
    ...
    // Enhance modules with full quiz questions
    if (course.modules) {
      for (const module of course.modules) {
        if (module.quizzes) {
          module.quizzes = await storage.getQuizzesByModule(module.id);  // This now returns full data!
        }
      }
    }
...
app.post('/api/modules/content/:id/complete', isAuthenticated, async (req: any, res) => {
  try {
    ...
    console.log(`[COMPLETE] User ${userId} marking content ${contentId} as complete`);

    // 1. Mark lesson as complete using lessonProgress table
    await storage.updateLessonProgress(userId, contentId, {
      lastPosition: 0,
      completed: true,
      watchTime: 0,
    });

    console.log('[COMPLETE] Content marked');

    // 2. Get courseId from contentId via moduleContents -> modules
    const { moduleContents, modules } = await import('@shared/schema');
    const { eq } = await import('drizzle-orm');

    const [contentRow] = await db.select().from(moduleContents).where(eq(moduleContents.id, contentId));

    if (!contentRow) {
      return res.status(404).json({ message: "Content not found" });
    }

    const [moduleRow] = await db.select().from(modules).where(eq(modules.id, contentRow.moduleId));

    if (!moduleRow) {
      return res.status(404).json({ message: "Module not found" });
    }

    const courseId = moduleRow.courseId;

    // 3. Calculate overall course progress
    const progress = await storage.getCourseProgress(userId, courseId);
    console.log('[COMPLETE] Course progress:', progress + '%');

    // 4. If 100% complete, generate certificate
  if (progress === 100) {
      console.log('[COMPLETE] Course complete - generating certificate');
      try {
        ...
        const alreadyHasCert = existingCerts.some((c: any) => c.courseId === courseId);
        ...
        if (!alreadyHasCert) {
          ...
          await db.insert(certificates).values({
            ...
            instructorName: 'Ume Urooj Gillani',
            completionDate: new Date(),
            issueDate: new Date(),
            verificationCode: `VER-${Date.now()}`,
          });
          console.log('[COMPLETE] Certificate generated for user:', userId);
        } else {
          console.log('[COMPLETE] Certificate already exists - skipping');
        }
      } catch (certErr: any) {
        console.error('[COMPLETE] Certificate generation error:', certErr.message);
      }
    }
    return res.json({
      success: true,
    ...
    console.error('[COMPLETE ERROR] Full details:', {
      message: err.message,
      stack: err.stack,
      contentId: req.params.id,
      userId: req.session?.user?.id,
    });
    return res.status(500).json({ message: 'Server error while marking complete' });
  }
});
  ...
  try {
    ...
    const { enrollments, users, courses } = await import("@shared/schema");
...
app.patch('/api/admin/enrollments/:id', isAuthenticated, isAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const { status, progress, accessLevel } = req.body;
  ...
  app.get("/api/quiz-attempts/:quizId", isAuthenticated, async (req: any, res) => {
  try {
    const { quizId } = req.params;
    ...
    // isAuthenticated middleware already ensures req.session.user exists
    const userId = (req.session as any).user.id;
 ...
 app.post('/api/lessons/:lessonId/progress', isAuthenticated, async (req: any, res) => {
  try {
    const userId = (req.session as any).user.id;
    const { lessonId } = req.params;
    const { lastPosition = 0, completed = false, watchTime = 0, courseId } = req.body;
    ...
    // If this lesson was just completed AND we know the course → check full course
    if (completed && courseId) {
      const courseProgress = await storage.getCourseProgress(userId, courseId);
      const quizzesPassed = await storage.areAllCourseQuizzesPassed(userId, courseId);

      // Course is finished when: 100% lessons + all quizzes passed
      if (courseProgress === 100 && quizzesPassed) {
        // Prevent duplicate certificates
        const existing = await storage.getUserCertificates(userId);
        const hasCert = existing.some((c: any) => c.courseId === courseId);
        ...
        if (!hasCert) {
          ...
          if (course) {
            ...
            console.log(`✅ Certificate AUTO-issued for user ${userId} - ${course.title}`);
          }
        }
      }

      // Return progress info to frontend
      return res.json({
        ...progress,
        courseProgress,
        courseCompleted: courseProgress === 100 && quizzesPassed,
      });
    }
  ...
  res.json([
    {
      ...
      isEnabled: true,
      name: 'SafePay',
      description: 'Pay with card or bank via SafePay',
      processingFee: '0',
      minAmount: '100'
    }  // ← added closing } here
  ]);  // ← added closing ] here
...
app.post('/api/payment/verify', isAuthenticated, async (req, res) => {
  try {
    ...
    const order = await storage.getOrderById(orderId.split('?')[0]);
...
//  Your existing payment transaction routes (unchanged)
// ────────────────────────────────────────────────
...
// Receipt management routes (unchanged)
...
// Payment history routes (unchanged)
...
// ────────────────────────────────────────────────
app.get('/payment/cancel', isAuthenticated, async (req, res) => {
  const { orderId } = req.query;

  console.log('[SAFEPAY] Payment cancelled:', { orderId });

  try {
    // TODO: Update order/transaction status to 'cancelled'
    // await storage.updatePaymentTransaction(orderId, { status: 'cancelled' });

    res.redirect('/payment/cancel?orderId=' + (orderId || 'unknown'));
  } catch (err) {
    console.error('[SAFEPAY] Cancel callback error:', err);
    res.redirect('/payment/error?message=failed_to_update_order');
  }
});
