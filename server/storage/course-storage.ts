    ...
    try {
      ...
      const modulesWithContent = await Promise.all(
        courseModules.map(async (module) => {
          ...
          const allContent = [...contentWithStatus];
      ...
      let progress = 0;
      if (userId) {
        const totalContent = modulesWithContent.reduce((sum, m) => 
          sum + m.content.filter((c: any) => c.type !== 'quiz').length, 0);
        const completedContent = modulesWithContent.reduce((sum, m) => 
          sum + m.content.filter((c: any) => c.type !== 'quiz' && c.completed).length, 0);
        progress = totalContent > 0 ? Math.round((completedContent / totalContent) * 100) : 0;
      }

      // Return the complete course data including progress
      return {
        id: course.id,
        title: course.title,
        description: course.description,
        ...
        enrolledCount: 0,
        progress, // ← progress now included
